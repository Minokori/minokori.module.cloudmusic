...
public class Converter {
     ...
     * @return 转换成功与否
     */
    public Tuple ncm2Mp3(String ncmFilePath, String outFilePath) {
        try {
            Ncm ncm = new Ncm();
            ...
            return new Tuple(true, ncm.getMata().format);
        } catch (Exception e) {
    ...
    private void combineFile(Ncm ncm) throws Exception {
        ...
        Bitmap image = BitmapFactory.decodeStream(new ByteArrayInputStream(ncm.getImage()));
        //BufferedImage image = ImageIO.read(new ByteArrayInputStream(ncm.getImage()));
        if (image != null) {
            MetadataBlockDataPicture coverArt = new MetadataBlockDataPicture(ncm.getImage(), 0, Utils.albumImageMimeType(ncm.getImage()), "", image.getWidth(), image.getHeight(), image.hasAlpha() ? 32 : 24, 0);
